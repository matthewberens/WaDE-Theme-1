...
st_write(EFPC_catchments_landuse, "GIS_data/EFPC_catchments_landuse.gpkg", append = FALSE)
...
Next, group the subcatchments together into one larger catchment that encompasses and entire tributary network for each site
```{r, message = FALSE, warning = FALSE}
...
#This code calculates the zonal statistics for each of the EFPC catchments. The results are exported as a df that is saved as 
landuse_bins <- exact_extract(nlcd_2019, catchments_binned, fun = 'frac', force_df = TRUE)

landuse_bins$site_name = catchments_binned$site_name

EFPC_catchments_binned <- merge(catchments_binned, landuse_bins, by = "site_name") %>%
...
#Export combined land use calculations with sampling locations
st_write(EFPC_catchments_binned, "GIS_data/EFPC_LU_catchments_binned.gpkg", append = FALSE)
