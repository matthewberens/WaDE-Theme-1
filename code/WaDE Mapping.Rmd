...
mapview(WaDE_sites, zcol = "network", layer.name = "Network", alpha = 1, col.regions = RColorBrewer::brewer.pal(length(unique(WaDE_sites$network)), "Spectral")) +
...
mapview(nlcd_2019, col.regions = data, att = "Class", layer.name = "Landcover Class", label = FALSE, legend = FALSE, alpha = 0.5, hide = TRUE)
...
sites <- st_intersection(merged, WaDE_sites)%>%
  dplyr::select(-c(NHDPlusID, SourceFC, GridCode, VPUID, SHAPE_Leng, SHAPE_Area, OBJECTID.1, AreaSqKm.1))
```
